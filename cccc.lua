...
function CheckIs(a)
    WorldOrigin = game.Workspace._WorldOrigin
...
A:AddToggle({Title = "Auto Raid [Flame]", Content = "Fully Raid", Default = false, Callback = function(vRaid)
    getgenv().Raid = vRaid
end
})
spawn(function()
    while task.wait() do
        ...
        if getgenv().Raid and (game.PlaceId == 4442272183 or 7449423635) then
            if IsRading() then
                local Table_Island = {5,4,3,2,1}
                for i,v in Table_Island do
                    if CheckIs(v) and GetDistance(CheckIs(v).Position) <= CurrentNumber then
                        ToTween(CheckIs(v).CFrame * CFrame.new(0,70,0))
                    end
